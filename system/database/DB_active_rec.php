...
//                       echo $this->ar_like[$key];
    ...
    function __like($field, $match = '', $type = 'AND ', $side = 'both', $not = '')
    {
        if (!is_array($field)) {
            $field = array($field => $match);
        }
...
//			$this->ar_like[] = substr($like_statement,4,strlen($like_statement)-4);
            $this->ar_like[] = $like_statement;
            if ($this->ar_caching === TRUE) {
                $this->ar_cache_like[] = $like_statement;
                $this->ar_cache_exists[] = 'like';
    ...
    function _or_like($field, $match = '', $side = 'both')
    {
        return $this->__like($field, $match, 'OR ', $side);
    }
