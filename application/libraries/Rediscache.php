...
class Rediscache
{
	...
	protected $_CACHE_KEY_PREFIX = '__APICACHE';
    ...
    */
    public function set($key, $value, $expire = 86400)
    {
        ...
        try {
			...
			$serializedVal	= base64_encode( serialize($value) );
	...
	{
		$ping = $this->CI->redis->command('PING');
		if($ping == 'PONG'){
			return true;
		} else {
			return false;
    ...
    */
    public function get($key)
    {
        ...
        if ($serializedResult) {
			return unserialize( base64_decode($serializedResult) );
        } else {
            return false;
        }
    }
