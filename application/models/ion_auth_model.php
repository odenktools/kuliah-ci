...
class Ion_auth_model extends CI_Model
{
     ...
     * @author Mathew
     **/
    public function hash_password($password, $salt = false, $use_sha1_override = FALSE, $use_pure_sha1 = FALSE)
    {
        if (empty($password)) {
            return FALSE;
        }
        ...
        } else if($use_pure_sha1 === FALSE && $this->store_salt && $salt){
            $salt = $this->salt();
            return $salt . substr(sha1($salt . $password), 0, -$this->salt_length);
        }else if($use_pure_sha1 === TRUE){
	...
	public function test_group($groups = array()){
		...
		//echo json_encode($default_group);
    ...
    public function register($username, $password, $email, $additional_data = array(), $groups = array())
    {
		...
		$config_group = $this->config->item('default_group', 'ion_auth');
		
		$table_group = $this->tables['groups'];
        ...
        if ($this->identity_column == 'email' && $this->email_check($email)) {
            ...
            return FALSE;
        } elseif (!$config_group && empty($groups)) {
            $this->set_error('account_creation_missing_default_group');
            return FALSE;
        }

        //check if the default set in config exists in database
        //$query = $this->where('name', $config_group)->group();
		
		$query = $this->db->query("select * from $table_group where name = '$config_group'");
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, mem_expire')
            ->where($this->identity_column, $identity)
            ->limit(1)
            ->get($this->tables['users']);
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                $purchase = $this->isPurchasableByUserTable($user->username) == '1' ? TRUE : FALSE;

                if ($purchase) {

                    if($user->mem_expire === NULL){

                        $idx = $this->get_users_groups($user->id)->row();

                        $this->set_expire_member( $user->id, $this->calculateDays($idx->id) );
                    }

                    if ($this->validateMembership($user->id) === 0) {

                        $this->trigger_events('post_login_unsuccessful');

                        $this->set_error('login_unsuccessful_expire');
     ...
     * @author Mathew
     **/
    public function restlogin($identity, $password, $remember = TRUE)
    {
        $this->trigger_events('pre_login');
     ...
     * @author aditya menon
     */
    public function create_group($group_name = FALSE, $group_description = '', $additional_data = array(), $active = 0, $purchaseable = 0, $amount = 0, $price = 0, $day = 0, $periode = 'D', $tipe = 'MEMBER')
    {
        ...
        $data = array('name' => $group_name, 'description' => $group_description,
            'active' => $active, 'purchaseable' => $purchaseable,
            'amount' => $amount, 'price' => $price,
            'day' => $day, 'periode' => $periode,
            'tipe' => $tipe
        );
     ...
     * @author aditya menon
     **/
    public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array(), $active = 0, $purchaseable = 0, $amount = 0, $price = 0, $day = 0, $periode = 'D', $tipe = 'MEMBER')
    {
        ...
        $data['purchaseable'] = $purchaseable;
...
//    }
     ...
     * @return bool|string
     */
    public function calculateDays($id)
    {
        $now = date('Y-m-d H:i:s');

        $this->db->select('day, periode');
        $this->db->where('id', $id);
     ...
     * @return bool
     */
    function set_expire_member($user_id, $expire_time)
    {

        $this->db->set('mem_expire', $expire_time);
    ...
    {
        $this->db->select('purchaseable');
        $this->db->where('id', $id);
        $query = $this->db->get($this->tables['groups']);

        $row = $query->row();

        return $row->purchaseable;
    }

    public function isPurchasableByUserTable($data)
    {
        $id = $this->identity_column;

        $query = "SELECT grp.purchaseable
                    FROM odk_groups grp
                      RIGHT JOIN odk_users_groups oug
                        ON oug.group_id = grp.id
                      INNER JOIN odk_users ou
                        ON ou.id = oug.user_id
                    WHERE ou. $id = '$data';";
